...
-   **USB BLEドングルはPCに直接接続する必要があるか？電源供給のみで動作するか？**
    → **PCに直接接続する必要があります**。電源供給だけでは動作しません。USB BLEドングルはPCのBLEスタックとUSB経由で通信するため、PCに接続してドライバ経由でBLE機能を提供します。PC側のソフトウェア（PythonのbleakやC#のWindows.Devices.Bluetooth）は、PCのBLEスタック経由でドングルを制御します。代替案として、PCに内蔵BLEアダプタがある場合はそれを使用できます（多くのノートPCや一部のデスクトップPCに内蔵）。
